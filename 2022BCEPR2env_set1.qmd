...
```{r}
sel_data_kp <- sel_data_kp %>% filter(use_trial_name %in% c("202240BCEPR_cere",  "202241BCEPR_stom"))
```
...
trial_standard <- trial_standard %>% 
  mutate(use_harvest_date = ifelse(use_trial_name == "202241BCEPR_stom", "2023-April-28", use_harvest_date))
```
...
ggsave(paste0("images\\boxplot_", trial_interest, Sys.Date(), ".png"),
    plot = plot_bxp, units = "in", dpi = 300, width = 14, height = 8
  )
```
...
# plant_type heritability from cere

```{r}
master_data$BLUP_BLUE_202240BCEPR_cere %>% 
  select(contains("plant_type")) %>% 
  ggplot(aes(x = BLUEs_plant_type, y = BLUPs_plant_type)) +
  geom_point()


my_dat %>% filter(str_detect(trial_name, "cere"), check_test == "test") %>% 
  select(accession_name, rep_number, plant_type) %>% 
  pivot_wider(names_from = "rep_number", values_from = "plant_type") %>% 
  ggplot(aes(x = `1`, y = `2`)) +
  geom_jitter()
...
```{r}
# Traits to remove because they are in only one trial and h2 < 0.1
...
ggsave(paste("images\\geno_corr", trial_interest, Sys.Date(), ".png", sep = "_"),
       plot = geno_cor, units = "in", dpi = 300, width = 10, height = 8)
```
...
names(master_data)[13] <- "BLUP_BLUE_root_commercial"
names(master_data)[21] <- "stability_germ_number_plot"
names(master_data)[24] <- "stability_root_commercial"
